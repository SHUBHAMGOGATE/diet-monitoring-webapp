...
class AddFoodViewContainer extends Component {
	...
	handleDoneAddingFoodsClick() {
		...
		const nextLocation =
			currentLocation.substring(0, currentLocation.indexOf("&mealgroupctx=")) +
			"&t=3";
	...
	render() {
			...
			cancelLinkContents = (
				<div className="container">
					<span className="FoodsPanel__context">
						Adding foods to <b>{this.state.mealGroupContext.name}</b>...
					</span>
					<button
						className="FoodsPanel__done-button"
						onClick={this.handleDoneAddingFoodsClick.bind(this)}
					>
		...
		return (
			<div className="AddFoodViewContainer">
				...
				/>
				<div className="FoodsPanel__cancel-link">{cancelLinkContents}</div>
			</div>
		);
	}
}
