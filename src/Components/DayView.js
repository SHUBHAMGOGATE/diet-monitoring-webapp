...
class DayView extends Component {
	...
	componentDidMount() {
		...
		let selectedDay = dayParam
			? this.convertQueryStringToDate(dayParam)
	...
	getConsumptions(day) {
		...
		fetch(
			deploymentConfig().apiUrl +
				"/api/consumptions?type=day&userId=" +
				userId +
				"&date=" +
				date +
				"&token=" +
			...
			if (res.ok) {
				...
				window.location.hash =
					deploymentConfig().baseUrl + "#/login?midreq=true";
			}
		});
	}
	...
	calculateDayTotals(mealTotals) {
		...
		let netCalories =
			caloriesEaten -
			(this.state.caloriesBurned ? this.state.caloriesBurned : 0);
		return {
	...
	handleServingUpdate(newItemsList, mealType) {
		...
		this.state.meals[mealType].items.forEach((item) => {
			...
			if (
				item.selectedServing.servingSize.id !==
					correspondingNewItem.selectedServing.servingSize.id ||
				item.selectedServing.quantity !==
					correspondingNewItem.selectedServing.quantity
			) {
		...
		if (updatedConsumption) {
			...
			fetch(
				deploymentConfig().apiUrl +
					"/api/consumptions/" +
					updatedConsumption.consumptionId +
					"?token=" +
					localStorage.getItem("token"),
				{
					method: "POST",
	...
	handleActivityChanged(calories) {
		...
		const userId = decodeToken(localStorage.getItem("token")).userId;
		this.apiPostTimeout = setTimeout(
			() => this.postActivityChange(calories, userId),
			500
		);
	}
