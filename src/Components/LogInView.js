...
class LogInView extends Component {
			...
			fetch(deploymentConfig().apiUrl + "/api/users/register", {
				...
				.then(res => {
					...
					if (res.error) {
						...
						// if no error, collect token and log user in
	...
	render() {
		...
		if (justLoggedOut) {
			welcomeText = (
				<span className="LogInView__notify LogInView__logout-confirm">
			...
			welcomeText = (
				<span className="LogInView__notify LogInView__session-expired">
					<span role="img" aria-label="Clock">
						⏲️
					</span>{" "}
					&nbsp;Your session expired. To keep things secure, please log back in.
				</span>
			);
		} else {
			welcomeText = (
		...
		if (!this.state.waiting) {
			submitButton = (
				<button
					type="submit"
					className={"logInButton" + (this.state.waiting ? " waiting" : "")}
				>
			...
			submitButton = (
				<img
					className="LogInView__form--loader"
					src={loader}
					alt="Loading..."
				/>
			);
		...
		if (this.state.registering) {
			switchText = (
				<span className="LogInView__form--switch">
					You're registering a new account. If you already have one, you can{" "}
					<span onClick={this.switchActionType.bind(this)}>log in</span>.
				</span>
			);
		} else {
			switchText = (
				<span className="LogInView__form--switch">
					...
					<span onClick={this.switchActionType.bind(this)}>Click here</span> to
					register.
				</span>
			);
		}
		...
		return (
			<div className="LogInView content-container">
				...
				<div className="LogInView__form">
					<h2>
						{this.state.registering ? "Register a new account" : "Log In"}
					</h2>
				...
				{this.state.errorMessage && (
					<span className="LogInView__notify LogInView__error">
