...
class StatisticsView extends Component {
	constructor(props) {
		...
		this.state = {
			dayFrom: moment(
				new Date(Date.now() - 60000 * new Date().getTimezoneOffset())
			)
				.subtract(1, "weeks")
				.toDate(),
	...
	refreshStats() {
		...
		fetch(
			deploymentConfig().apiUrl +
				...
				userId +
				"&dateFrom=" +
				this.state.dayFrom.toISOString() +
				"&dateTo=" +
				this.state.dayTo.toISOString() +
				"&token=" +
				localStorage.getItem("token")
		).then(res => {
			if (res.ok) {
	...
	render() {
		...
		if (this.state.refreshing) {
			refreshButton = (
				<img
					className="StatisticsView__refresh-button refreshing"
					src={loaderGray}
					alt="Refreshing..."
				/>
			);
			...
			if (this.state.refreshButtonActive) {
				refreshButton = (
					<img
						className="StatisticsView__refresh-button"
						src={refreshIcon}
						onClick={this.refreshStats.bind(this)}
						alt="Refresh"
					/>
				);
			} else {
				refreshButton = (
					<img
						className="StatisticsView__refresh-button inactive"
						src={refreshIconInactive}
						alt=""
					/>
				);
			}
			...
			<div className="StatisticsView content-container">
				...
				<div className="StatisticsView__week-select">
					...
					<DayPickerInput
						...
						parseDate={parseDate}
						format="l"
						onDayChange={this.handleToDayChange.bind(this)}
					/>
					{refreshButton}
				</div>
